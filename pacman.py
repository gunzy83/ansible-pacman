...
class PacmanPackage:
    ...
    def query_package(self, module, pacman_path):
        lcmd = "%s -Qi %s" % (pacman_path, self.package_name)
        ...
        rcmd = "%s -Si %s" % (pacman_path, self.package_name)
...
def remove_packages(module, pacman_path, packages):
    ...
    for package in packages:
        ...
        cmd = "%s -%s %s --noconfirm --noprogressbar" % (pacman_path, args, package.get_package_name())
        rc, stdout, stderr = module.run_command(cmd, check_rc=False)
...
def install_packages(module, pacman_path, packages, state, as_deps):
    ...
    if to_install_repos:
        ...
        cmd = "%s -S %s --noconfirm --noprogressbar --needed" % (pacman_path, package_string)
        rc, stdout, stderr = module.run_command(cmd, check_rc=False)
    ...
    if to_install_files:
        ...
        cmd = "%s -U %s --noconfirm --noprogressbar --needed" % (pacman_path, package_string)
        rc, stdout, stderr = module.run_command(cmd, check_rc=False)
    ...
    if update_install_reason:
        ...
        cmd = "%s -D %s %s" % (pacman_path, package_string, operation)
        rc, stdout, stderr = module.run_command(cmd, check_rc=False)
